...
| `threads`          | `--threads`         | 1 (default), 0 = every core, 2…        | **Parallel stroke placement.** `1` = the classic single painter, byte‑identical to every accepted render (and ~3× faster than before: the pigment solver is allocation‑free). `0`/`2+` = the checkerboard **tile schedule**: ~13× on 18 cores, the same picture for any thread count ≥ 2, replay‑exact from its score — but a different hand from the single order (same look, different micro‑texture). Not recorded (a schedule, not a mark). Set `PLAKAT_PAINT_PROFILE=1` to print per‑pass timings to stderr. |
