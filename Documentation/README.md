...
| [`TEXTURE.md`](TEXTURE.md) | **v6.3 flagship (RFC TEXTURE-1).** `plakat texture` — turn a prompt or a photo into a *seamless, tileable PBR material set* (albedo/normal/roughness/metallic/height/AO), flat-lit, exported engine-ready (ORM pack, Unity/Unreal naming, glTF) with a pure-Rust lit preview. The `TextureSpec` schema, the derive/scorecard/preview/export pipeline (the whole weight-free half), the seamless + delight + depth-height + tiled-upscale concepts, and the integration surfaces (scenario / compile / Bund / `plakat::api::Texture`). Companions: [`Tutorials/TEXTURE_TUTORIAL.md`](Tutorials/TEXTURE_TUTORIAL.md), [`RFC_TEXTURE_1.md`](RFC_TEXTURE_1.md). |
