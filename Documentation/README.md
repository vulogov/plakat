...
| [`TEXTURE.md`](TEXTURE.md) | **v6.3 flagship (RFC TEXTURE-1), deepened in v6.4.** `plakat texture` — turn a prompt or a photo into a *seamless, tileable PBR material set* (albedo/normal/roughness/metallic/height/AO), flat-lit, exported engine-ready (ORM pack, Unity/Unreal naming, glTF) with a pure-Rust lit preview. The `TextureSpec` schema, the derive/scorecard/preview/export pipeline (the whole weight-free half), the seamless + delight + depth-height + tiled-upscale concepts, and the integration surfaces (scenario / compile / Bund / `plakat::api::Texture`). **v6.4:** spatially-varying `metallic`/`roughness: "auto"` (structured masks for composites), *Reading the channels* (why a flat map is correct), anisotropy, `texture blend`, `--variations`, painted-mask refs. Companions: [`Tutorials/TEXTURE_TUTORIAL.md`](Tutorials/TEXTURE_TUTORIAL.md), [`RFC_TEXTURE_1.md`](RFC_TEXTURE_1.md). |
| [`BOOKART.md`](BOOKART.md) | **v6.0/6.1 flagship (RFC BOOKART-1).** `plakat bookart` — compose *reusable, print-ready, transparent black-and-white book ornaments* from a small HJSON spec, in a chosen illustration tradition × technique, at an exact page size. The ornament vocabulary, the `BookArtSpec` schema, the hybrid render router (procedural / diffusion / composite), the print/ink scorecard, the full command reference (`render`/`illustrate`/`kit`/`manuscript`/`origins`/`vectorize`/`font`/`diff`/`edit`/`blend`), and the 6.1 integration surfaces (scenario / compile / Bund / `plakat::api::BookArt` / `--import`). Companions: [`BOOKART_TRANSPARENCY.md`](BOOKART_TRANSPARENCY.md) (the B/W-native alpha model + print sizing + SVG trace), [`BOOKART_STYLES.md`](BOOKART_STYLES.md) (origins × techniques + the six hosted origin LoRAs), [`Tutorials/BOOKART_TUTORIAL.md`](Tutorials/BOOKART_TUTORIAL.md), [`RFC_BOOKART_1.md`](RFC_BOOKART_1.md). |
...
| [`SCRIPTING.md`](SCRIPTING.md) | **v0.21+.** `plakat run SCRIPT.bund` Bund scripting reference. The `plakat.*` host words grouped by namespace (core image surface + `lora`/`controlnet`/`refiner`/`style`/`look`/`genre`/`portrait`/`embedding`/`metadata`/`map`/`fractal`/`bookart` …), the full `GenerationConfig` knob list, REPL meta-commands, architecture notes. Companion to [`Tutorials/SCRIPTING_TUTORIAL.md`](Tutorials/SCRIPTING_TUTORIAL.md). |
