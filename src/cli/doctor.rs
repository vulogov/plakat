...
fn section_quality() {
    section_header("naturalize / quality (6.10–6.13 — de-slop AI images: a better picture)");
    ...
    note("6.13 --repair figure-SCOPED (default `--repair-scope figures`): projects a body box from each face → repaints only the FIGURES, preserving faces AND background (fixes the 6.12 background-changed/colour-drift regression). `non-face`/`full` scopes remain. 6.13 AUTO-MEDIUM: --repair/--geometry without --style CLIP-zero-shot the source medium so the re-paint holds it.");
