...
fn section_texture() {
    ...
    note("engine export (6.6): `texture export --engine gltf|unreal|unity-hdrp|godot|materialx|plakat` picks naming + packing + material doc in one shot. Packings DIFFER: ORM=R:AO/G:rough/B:metal (glTF/Unreal/Godot) vs Unity HDRP mask map=R:metal/G:AO/B:detail/A:smoothness. glTF emits KHR_materials_anisotropy from the anisotropy flow map; MaterialX emits a 1.38 standard_surface. Also on `render`/`derive` via `--engine` and `api::{Texture::engine, texture_export}`.");
...
fn section_comic() {
    ...
    note("multi-page (6.8.1): `pages: [{layout,panels}]` — top-level cast/style/model/seed = the shared world propagated to every page → `out_00.png, out_01.png, …`; `scenes: {alley:\"…\"}` named library (`scene: \"@alley\"`) recurs a setting; `extends: \"series.hjson\"` inherits a base spec. Global panel-seed counter across pages.");
    note("reference-lock (6.8.1 M2): `comic cast` builds a per-character reference sheet (`ref_<name>.png`); `render --lock` face-swaps that reference onto every panel so identity holds book-wide — best-effort, needs the face-swap weights (SCRFD+ArcFace+inswapper), else stays description-level. `style_lora` (+ `style_lora_scale`) locks the look on every panel; explicit `cast[].reference` overrides the rendered portrait; `cast[].lock: false` opts a character out.");
...
fn section_quality() {
    section_header("naturalize / quality (6.10–6.16 — de-slop AI images: a better picture)");
    ...
    note("content focus qualifiers (weight-free, combine): --people (plastic skin → micro) · --sky · --vegetation · --cityscape · --landscape · --sea · --river · --mechanics · --household · 6.13: --animal · --food · --interior · --textile · --foliage-macro <N>. Each blends a subject-specific profile.");
    note("6.13 --paper N (weight-free, ART ONLY): watercolor paper/pigment authenticity — paper TOOTH + GRANULATION + edge POOLING, pigment-gated (photos untouched). Fixes the 'simulated media' tell. Recommended ~0.6 (subtle 0.5 · hand-painted 1.0+).");
    note("6.13 --repair figure-SCOPED (default `--repair-scope figures`): projects a body box from each face → repaints only the FIGURES, preserving faces AND background. `non-face`/`full` scopes remain. 6.14: also OWL-ViT person-detect so FACELESS figures are covered. AUTO-MEDIUM (6.13): --repair/--geometry without --style CLIP-zero-shot the source medium; `--auto-medium` opts a weight-free run in.");
    note("6.14 auto-paper: wet media (watercolor/gouache/ink-wash, named or auto-detected) auto-applies --paper 0.6 (--paper 0 disables). --paper reachable via spec (`generate --naturalize \"... paper=0.6\"`/scenario/api). 6.14 BATCH: a directory input de-slops every image into the --out dir.");
    note("6.15 --report: a de-slop SCORECARD (plakat's own AI-tell verdict — oversaturation + over-smoothness decomposed, CLIP medium, a recommended recipe) instead of processing; --json for structured. 6.15 --auto-regions / --region \"x0,y0,x1,y1:spec\": per-region focuses (sky band + SCRFD people), feathered-composited.");
    note("6.15 VIDEO de-slop: a video/animation input (mp4/mov/webm/mkv/avi/gif) de-slops EVERY frame + re-encodes (weight-free; grain is frame-invariant → no flicker). Needs ffmpeg.");
    note("6.16 FOLDER scorecard: `naturalize <dir> --report` ranks a whole folder worst-AI-first + aggregate summary. 6.16 LUT: `naturalize --export-lut grade.cube [--lut-size 33]` bakes the fixed grade (desaturate+warm) to a .cube for Resolve/Premiere/OBS (WB/auto-levels are per-image, not in the LUT). 6.16 PRESET LIBRARY: `--list-presets` + `--preset portrait|landscape|product|anime|film|restore`.");
