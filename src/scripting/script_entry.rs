...
pub fn generate_one(ctx: &mut ScriptCtx, prompt: &str) -> Result<DynamicImage> {
    ...
    match PipelineFamily::detect(&alias) {
        ...
        PipelineFamily::Flux => {
            ...
            // v0.26 phase 10: --look / --genre apply + auto-LoRA
            // discovery on the Flux generate path. Mirrors the
            // v0.25 phase 8 SD-family block, with two differences:
            //   - Style doesn't run on Flux (bailed above) so the
            //     preset is the only prompt-prefix source.
            //   - Flux is guidance-distilled (no CFG) so the
            //     preset's negative_extras is silently ignored at
            //     inference time. Still collected here for
            //     parity but never threaded into req.negative.
            ...
            if ctx.look_name.is_some() || ctx.genre_name.is_some() {
                ...
                effective_prompt = params.prompt;
                // params.negative discarded — Flux ignores negatives
                // at inference (no CFG batching).
                if let Some(s) = params.steps {
                    ctx.config.steps = s;
                }
                if let Some(g) = params.guidance {
            ...
            let req =
                build_flux_gen_request(ctx, &effective_prompt, tmp_path.clone(), None);
