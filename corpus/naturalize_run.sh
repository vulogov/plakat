#!/usr/bin/env bash
#
# QUALITY-1 comprehensive driver (RFC QUALITY-1, plakat 6.10) — demonstrates EVERY `naturalize` feature.
#
# `naturalize` reduces the "AI-generated" fingerprint of an image. The analog half (presets, content
# focuses, ghost-signature removal, AI-tell score) is weight-free — no GPU. The corrective focuses
# (geometry / anatomy / no-twins) and the `generate` integration (`--quality`, `--naturalize`) need a
# model; run with RENDER=1 to include them. All outputs land under corpus/images/quality/.
#
...
if [ "$RENDER" = "1" ]; then
  ...
  # 7b. generate integration — the --quality preset and the --naturalize post-pass.
  ...
  echo "· RENDER=0 — skipping model-backed features (corrective geometry/anatomy/no-twins · generate --quality/--naturalize)."
