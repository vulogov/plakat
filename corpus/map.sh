...
grep -q '"class": "territory"' "$ROOT/corpus/map/export/realms.geojson" \
  || { echo "✗ realms GeoJSON is missing the territory polygons (1.14.0-D)"; exit 1; }
...
# D2) Per-tile furniture (--map-tile-furniture): each tile gets a frame + grid
#     coordinate (R1C2) + north arrow so it stands alone. Byte-stable, opt-in.
"$PLAKAT" map --map-spec "$REALMS" --seed 42 --map-render-tiles "$T-realms-furn" --map-tile-furniture >/dev/null
for r in 0 1 2; do for c in 0 1 2; do
  check "$T-realms-furn/tile_r${r}_c${c}.png" "$OUT/realms-tiles-furnished/tile_r${r}_c${c}.png" "furnished tile r${r}c${c}"
done; done
echo "✓ D2 per-tile furniture (frame + R/C coordinate + north arrow) byte-stable"
...
rm -rf "$T"-*.png "$T"-*.json "$T".geojson "$T".svg "$T-realms.geojson" "$T-realms.svg" \
       "$T-realms-tiles" "$T-realms-furn" "$T-realms.json"
