...
| Segment / select (SAM) | `segment.sh` | ✅ | ⬜ | **v1.0 — the compose-&-edit enabler.** `plakat segment` — click an object with point prompts (`--point X,Y[:bg]`, normalized or pixel) → a binary mask via **MobileSAM** (candle-transformers TinyViT, ~40 MB ungated `vulogov98/mobile-sam`, ~0.4 s on Metal). Mask feeds the existing `--mask` consumers. Driver: select the portrait subject (3 pts) → `--invert` → `img2img --mask` repaints a new background, subject preserved. `bg-mask.png` committed; the inpaint stage is GPU (⬜ render). |
