...
| **DreamBooth subject LoRA (SD3.5)** | `dreambooth_sd35.sh` | ✅ | ⬜ | **v1.1.** Same DreamBooth but on the SD3.5 **MMDiT** trainer — prior-preservation class loss ported to the rectified-flow objective (`v=ε−x₀`, independent class σ/noise, λ-weighted). Subject (teal dragon plush) + class sets generated with a LIGHT base (sd15) so SD3.5 is paid only for training (256²) + renders. `--base sd35 --class-dir/--class-prompt --prior-weight`. ⚠️ SD3.5 memory-heavy: native 1024² renders may OOM 24 GB (`SIZE=768x768` to lighten). Verification: render pending (memory-bound, like sd21/sdxl). |
