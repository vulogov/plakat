...
| **Resume training (`--resume`)** | `resume_train.sh` | ✅ | ⬜ | **v1.0**. Train a short run with numbered checkpoints → resume from one → continue + render. Proves `plakat style train --resume …-step<N>.safetensors --steps M` reloads the adapters and continues the counter (all bases; sd35's fused-qkv PEFT inverse is unit-tested). Strictly additive — no `--resume` = train from scratch as before. |
