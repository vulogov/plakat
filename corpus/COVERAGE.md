...
| **Regional prompting (`--region`)** | `regional.sh` | ✅ | ⬜ | **v1.0 — Part 1.** `plakat generate "<base>" --region "x0,y0,x1,y1:prompt" …` — different prompts in different canvas regions of ONE image (MultiDiffusion: per-region noise predictions blended by bbox masks over the base). Driver: left = alpine, right = tropical → one golden-hour scene. SD 1.5 / SDXL, native res; reuses the verified tiled blend. 2 unit tests (parse + mask). |
