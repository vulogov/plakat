...
| Compile (prose → scenario) | `compile.sh` + `compile/basic.txt` | ✅ | ✅ | **v1.2 — Track C, COMPILE-1.** `plakat compile prompts.txt` → scenario HJSON: blank-line blocks (free text + `key: value` commands) → one task per block; global→scene inheritance with per-command merge (concatenate / accumulate / last-wins); model-family-aware prompt profiles (SD15/SDXL/Flux); auto-negatives from the enhanced positive; `translate:` (LLM pre-pass) + `persona:` (fragments from `~/.config/plakat/personas/`); `--lint` / `--dry-run`; two-namespace `--compile-cache` (+ `--compile-cache-clear`); `--diff` (per-task add/change/remove) + `--decompile` (scenario→prompts.txt); `scenario -` stdin pipe. **Verified deterministic**: `--no-enhance --no-negative` → **byte-stable** `basic.hjson` (committed), validated via `scenario --dry-run`, pipe, a no-op `--diff` guard, and a `--decompile` round-trip. **v1.3 (COMPILE-2):** optional **Tera template pre-pass** (`--features templates`) — `.tera`/`.j2` inputs render to a `prompts.txt` first, with `--var`/`--vars <json\|toml>`/`--vars-env` context, custom filters/functions (`scene_name`, `prompt_join`, `model_family`, `include_raw`, …), and `--dump-rendered[-only]`. Proof: `series.tera` + `series.json` → **byte-stable** `series.rendered.txt` → scenario-validated. No GPU / network / key. LLM path reuses the `--enhance` provider stack (`prompt::complete`). |
