...
| **DreamBooth subject LoRA** | `dreambooth.sh` | ✅ | ⬜ | **v1.0 — Part 2.** Learn a SUBJECT (not a style) + class prior-preservation. **Self-contained**: GENERATES a synthetic subject (orange fox plush, 4 views) + a class set (6 plushies), trains `style train --class-dir/--class-prompt --prior-weight`, then renders the subject (`sks` token) in new scenes — the learned plush should reappear. sd15, 256² train. ⚠️ ~1–2 h (DreamBooth doubles per-step; Metal sd15 training is slow). |
