...
| Artefact compositing (multi) | `artefact.sh` + `artefact.hjson` | ✅ | ⬜ | `--artefact NAME@ZONE --artefact-blend` — alpha-composite cutout PNGs into named zones, then a masked img2img pass blends them in. Driver builds a REAL SDXL library (balloon/pine/cottage via generate→transparent) vs the weak bundled *drawn* set, then composites a multi-artefact valley — CLI **and** scenario (`artefacts: [...]`). SDXL only (blend = SD-core img2img). |
