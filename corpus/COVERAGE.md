...
| **Regional prompting (`--region`)** | `regional.sh` | ✅ | ⬜ | **v1.0 — Part 1.** `plakat generate "<base>" --region "x0,y0,x1,y1:prompt" …` — different prompts in different canvas regions of ONE image (MultiDiffusion: per-region predictions blended by bbox masks over the base). Driver: left = alpine, right = tropical. **SD 1.5 / SDXL** (UNet noise blend) **+ SD3.5** (MMDiT velocity blend, `predict_velocity_regional`); native res, reuses the verified tiled blend. Also a per-task **`regions:` scenario key** (all the above models). 2 unit tests (parse + mask). |
