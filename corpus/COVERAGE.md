...
| **Textual Inversion training** | `embedding_train.sh` | ✅ | ✅ | **v1.1 — VERIFIED.** `plakat embedding train` learns ONE new token embedding (a "word") from a few images, model FROZEN (only the placeholder vector trains, spliced into the CLIP-L forward). SD 1.5 / 2.1. Self-contained: GENERATES a stained-glass style set → trains (1000 steps, **~1m43s** — single vector) → loads via `--embedding PATH:trigger` → renders new subjects. **Verified:** `a sgwin cat` clearly takes the learned luminous jewel-glass look (a plain cat never does). TI is subtle (1-vector capacity) — strength varies by subject (the locomotive took it less); more steps/vectors strengthen it. SDXL = follow-up. |
