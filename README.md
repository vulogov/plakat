...
> **v6.24.0 — History quick-actions in `plakat ui` (RFC UI-GALLERY-1)**: the **History** tab (`Ctrl-4`) becomes a workbench, not just a browser. On the selected frame: **`n`** naturalizes it in place (instant weight-free de-slop → `<name>_naturalized.png`), **`y`** queues fresh-seed variations from its embedded recipe, and **`Delete`** moves it to `out/.trash/` (recoverable, confirm-twice). (The thumbnail **grid** `v`, live **filter** `/`, and **semantic search** `?` were already there.) [Release notes →](https://github.com/vulogov/plakat/releases/tag/v6.24.0) · [UI tutorial →](Documentation/Tutorials/UI_TUTORIAL.md)
>
