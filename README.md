...
> **v6.24.0 — History quick-actions in `plakat ui` (RFC UI-GALLERY-1)**: the **History** tab (`Ctrl-4`) becomes a workbench, not just a browser. On the selected frame: **`n`** naturalizes it in place (instant weight-free de-slop), **`y`** queues variations, **`e`** etches provenance, and **`Delete`** trashes it (recoverable, confirm-twice). And — no new tabs — **Chat** gains **`/relight <preset>`** and **`/faceswap <src>`** to edit the latest frame with a model op (they reuse Chat's pipeline; result posts back to the thread). (The thumbnail **grid** `v`, live **filter** `/`, and **semantic search** `?` were already there.) [Release notes →](https://github.com/vulogov/plakat/releases/tag/v6.24.0) · [UI tutorial →](Documentation/Tutorials/UI_TUTORIAL.md)
>
